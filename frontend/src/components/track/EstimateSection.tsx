...
export function EstimateSection({ taskId }: { taskId: string }) {
  ...
  const doImport = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast('success', t('estimate.saved'))
    },
    onError,
  })
  const apply = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast(
        'success',
        t('estimate.applied', {
          value: fmtMinutes(result.estimate.likelyMinutes),
          count: result.summary.createdBlocks,
        }),
      )
    },
    onError,
  ...
  return (
    <section className="rounded-lg border border-gray-200 bg-white p-4">
      <h3 className="mb-3 text-sm font-semibold">{t('estimate.title')}</h3>
      <div className="flex flex-col gap-2">
        {parsable.length > 0 && (
          <div className="text-xs">
            <p className="mb-1 text-gray-500">{t('estimate.selectAttachments')}</p>
            {parsable.map((att) => (
        ...
        >
          {generate.isPending ? t('common.generating') : t('estimate.generatePrompt')}
        </button>
        {prompt && (
          <div>
        ...
        <textarea
          ...
          placeholder={t('estimate.replyPlaceholder')}
        ...
        >
          {t('estimate.validateSave')}
        </button>
      </div>

      <h4 className="mt-4 mb-2 text-xs font-semibold text-gray-600">{t('estimate.history')}</h4>
      <ul className="flex flex-col gap-2">
        {estimates.map((est) => (
          <li key={est.id} className="rounded-md bg-gray-50 p-2 text-xs">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {expandedId === est.id ? t('common.collapse') : t('common.detail')}
              </button>
            </div>
            {expandedId === est.id && (
              <div className="mt-2 flex flex-col gap-1 text-gray-600">
                {est.breakdown.length > 0 && (
                  <p>{t('estimate.steps', { text: est.breakdown.map((b) => `${b.step} ${fmtMinutes(b.minutes)}`).join('；') })}</p>
                )}
                {est.assumptions.length > 0 && <p>{t('estimate.assumptions', { text: est.assumptions.join('；') })}</p>}
                {est.risks.length > 0 && <p>{t('estimate.risks', { text: est.risks.join('；') })}</p>}
                {est.sourceAttachmentIds.length > 0 && (
                  <p>{t('estimate.sourceAttachments', { ids: est.sourceAttachmentIds.join(', ') })}</p>
                )}
              </div>
            )}
          </li>
        ...
        {estimates.length === 0 && (
          <p className="text-xs text-gray-400">{t('estimate.empty')}</p>
        )}
      </ul>
    </section>
  )
